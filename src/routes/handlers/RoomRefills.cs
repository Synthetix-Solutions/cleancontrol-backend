...
public static class RoomRefills {
	...
	/// <remarks>
	/// This method deletes a room refill from the database. It does this by finding the room refill in the database using the provided refill ID.
	/// If the room refill is not found, it returns a <see cref="NotFound"/> result.
	/// If the room refill is found, it removes it from the database and saves the changes, then returns an <see cref="Ok"/> result.
	/// </remarks>
	/// <returns>A <see cref="Results{T1, T2}"/> object that contains either an <see cref="Ok"/> result if the room refill was successfully deleted, or a <see cref="NotFound"/> result if the room refill was not found.</returns>
...
/// <param name="db">The database context.</param>
/// <remarks>
/// This method retrieves a specific room refill from the database. It does this by finding the room refill in the database using the provided room ID and refill ID.
/// If the room refill is not found, it returns a <see cref="NotFound"/> result.
/// If the room refill is found, it returns an <see cref="Ok{T}"/> result with the room refill.
/// </remarks>
/// <returns>A <see cref="Results{T1, T2}"/> object that contains either an <see cref="Ok{T}"/> result with the room refill, or a <see cref="NotFound"/> result if the room refill was not found.</returns>
...
/// <param name="db">The database context.</param>
/// <remarks>
/// This method adds a list of room refills to the database. It does this by finding the room in the database using the provided room ID,
...
/// If the room is found and the refills are successfully added, it saves the changes to the database and returns an <see cref="Ok"/> result.
/// </remarks>
/// <returns>A <see cref="Results{T1, T2}"/> object that contains either an <see cref="Ok"/> result if the refills were successfully added, or a <see cref="ProblemHttpResult"/> with an error message if the room was not found or there was an error adding the refills.</returns>
	...
	/// <param name="db">The database context.</param>
	/// <remarks>
	/// This method retrieves all room refills from the database. It does this by selecting all room refills from the database and returning them as a list.
