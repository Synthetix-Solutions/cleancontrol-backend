...
public static class RoomTasks {
	...
	/// <param name="db">The database context.</param>
	/// <remarks>
	///     This method retrieves a specific room task from the database. It does this by finding the task in the database
	///     using the provided task ID.
	///     If the task is not found, it returns a <see cref="ProblemHttpResult" /> with a 404 status code and an error
	///     message.
	///     If the task is found, it returns an <see cref="Ok{T}" /> result with the task.
	/// </remarks>
	/// <returns>
	///     A <see cref="Results{T1, T2}" /> object that contains either an <see cref="Ok{T}" /> result with the task, or
	///     a <see cref="ProblemHttpResult" /> with an error message if the task was not found.
	/// </returns>
	...
	/// <param name="db">The database context.</param>
	/// <remarks>
	///     This method retrieves all due room tasks from the database. It does this by finding the room in the database using
	...
	///     message.
	///     If the room is found and the tasks are successfully retrieved, it returns an <see cref="Ok{T}" /> result with the
	///     list of tasks.
	/// </remarks>
	/// <returns>
	///     A <see cref="Results{T1, T2}" /> object that contains either an <see cref="Ok{T}" /> result with the list of
	///     tasks, or a <see cref="ProblemHttpResult" /> with an error message if the room was not found.
	/// </returns>
