...
public static class CleaningRunsEndpoints {
	...
	private static Ok<CleaningRun> DeleteProduct(Guid id) => TypedResults.Ok<CleaningRun>(null);

	private static Ok<CleaningRun> UpdateProduct(Guid id) => TypedResults.Ok<CleaningRun>(null);

	private static Ok<CleaningRun> GetProduct(Guid id) => TypedResults.Ok<CleaningRun>(null);

	private static Ok<CleaningRun> CreateProduct() => TypedResults.Ok<CleaningRun>(null);

	private static Ok<IEnumerable<CleaningRun>> GetAllProducts() => TypedResults.Ok<IEnumerable<CleaningRun>>(null);
}
